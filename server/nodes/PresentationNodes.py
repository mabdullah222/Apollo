...
class Nodes:
    ...
    def QuizAgent(self, state: PresentationState) -> PresentationState:
        slides = state['slides']
        topic = state['topic']
        quiz = []
        ...
        for i in range(len(slides)):
            ...
            Instructions:
            1- Based strictly on the following slide, determine if a multiple choice question (MCQ) can be generated.
            2- Return ONLY a valid JSON array of objects as plain text.Dont add any language indicator like json or any other extra information.
            ...
            **Example Output:**
            ...
            response = self.llm2.invoke(message).content.strip()

            try:
                mcq_data = json.loads(response)
                quiz+=mcq_data
            except json.JSONDecodeError:
                print(f"Warning: Slide {i} returned invalid JSON")

        state['quiz'] = quiz
        print("Quiz Agent Complete")
        return state
