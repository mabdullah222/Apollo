...
def generate_lecture():
    ...
    async def process():
        try:
            ...
            for mcq in final_state['quiz']:
                await db.quiz.create(data={"lecture_id":lecture_in_db.id,"statement":mcq['statement'],"choices":mcq['choices'],"correct_choice":mcq['answer']})
...
def lecture_status(lecture_id):
    async def process():
        await db.connect()
        lecture = await db.lecture.find_unique(where={"id": lecture_id},include={"slide":True,"mcqs":True})
        await db.disconnect()
        ...
        return {
            ...
            "mcqs":[{"statement":mcq.statement,"choices":mcq.choices,"answer":mcq.correct_choice} for mcq in lecture.mcqs]
        }
...
def quiz(lecture_id):
    async def process():
        await db.connect()
        lecture = await db.lecture.find_unique(where={"id": lecture_id},include={"mcqs":True})
        await db.disconnect()
